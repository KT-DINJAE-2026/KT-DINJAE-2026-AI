...
## 실험 결과 추이 (300 → 1000 → 2000 → 3000트리)

서초구 4월 한 달치(938만 행) 동일 데이터 기준, 트리 수를 늘려가며 확인:

| | 300트리 | 1000트리 | 2000트리 | 3000트리 |
| --- | --- | --- | --- | --- |
| 모델A AUC | 0.9374 | 0.9447 | 0.9476 | **0.9489** |
| 모델A Accuracy | 92.84% | 93.07% | 93.17% | **93.22%** |
| 모델B MAE | 127.2초 | 121.4초 | 119.1초 | **117.8초** |
| 모델B RMSE | 186.5초 | 178.7초 | 175.5초 | **173.6초** |

트리 수를 늘릴수록 전 지표가 꾸준히 개선되고 있지만, 구간별 개선 폭은 계속 줄어드는 추세입니다(300→1000 대비 2000→3000 구간의 개선폭이 눈에 띄게 작아짐 — 수확체감 구간 진입). 트리 수만 더 늘리는 것보다 `learning_rate`, `num_leaves` 등 다른 하이퍼파라미터 조정으로 넘어갈 시점으로 판단하고 있습니다.
...
- 실험 결과 히스토리는 `experiments/` 폴더에 타임스탬프별로 누적 (최신: `experiments/26.08.03.16-03-50`)
