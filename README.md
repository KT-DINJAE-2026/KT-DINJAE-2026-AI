...
## 실험 결과 추이 (300 → 1000 → 2000 → 3000 → 4000 → 5000트리)
...
트리 수를 늘릴수록 지표가 계속 개선되고는 있지만, 4000→5000 구간의 개선폭은 이제 소수점 셋째 자리 수준까지 줄어들었습니다 — 트리 수만으로 얻을 수 있는 이득은 사실상 한계에 가까워진 것으로 판단됩니다. 다음 실험부터는 트리 수 증가 대신 `learning_rate`, `num_leaves` 등 다른 하이퍼파라미터 조정으로 전환할 계획입니다.
...
- 실험 결과 히스토리는 `experiments/` 폴더에 타임스탬프별로 누적 (최신: `experiments/26.08.03.17-51-21`)
